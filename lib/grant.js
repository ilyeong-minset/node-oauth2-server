...
function checkGrantType (done) {
  ...
  switch (this.grantType) {
    ...
    case 'public_key':
      return usePublicKeyGrant.call(this, done);
    case 'refresh_token':
...
function usePasswordGrant (done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    ...
    if (err) {
      if (err.message == 'invalid_password') return done(error('invalid_password', false, err));
      else return done(error('server_error', false, err));
    }
    if (!user) {
      return done(error('invalid_user', 'email doesn\'t exist'));
    }
...
function usePublicKeyGrant (done) {
  // User credentials
  var publicKey = this.req.body.publicKey,
    signature = this.req.body.signature,
    message = this.req.body.message;

  if (!publicKey || !signature) {
    return done(error('invalid_client',
      'Missing parameters. "publicKey" and "signature" are required'));
  }

  var self = this;
  return this.model.getPublicKeyUser({publicKey, message, signature}, function (err, user) {

    if (err) {
      if (err.message == 'invalid_signature') return done(error('invalid_signature', false, err));
      else return done(error('server_error', false, err));
    }
    if (!user) {
      return done(error('invalid_user', 'publicKey doesn\'t exist'));
    }
...
function updateNonce (done) {
  this.model.updateNonce(this.user, function(err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
